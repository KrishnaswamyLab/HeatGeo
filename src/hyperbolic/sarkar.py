...
from .multiprecision import poincare_reflect0
    ...
    while place:
        ...
        for w in tree[v]:
            ...
            if weighted:
                ...
                r = mpm.tanh(tau)
            w_emb = (
                r * mpm.matrix([mpm.cos(arg + theta * i), mpm.sin(arg + theta * i)]).T
            )
...
def sarkar_embedding_3D(tree, root, **kwargs):
    ...
    for i, v in enumerate(tree[root]):
        r = mpm.tanh(tau * tree[root][v].get("weight", 1.0))
    ...
    while place:
        ...
        R = rotate_3D_mp(mpm.matrix([[0.0, 0.0, 1.0]]), u_emb)
        # u_emb = (R.T * u_emb).T
        ...
        for w in tree[v]:
            if w == u:  # i=0 is for u (parent of v)
                continue
            i += 1
            r = mpm.tanh(tau * tree[w][v].get("weight", 1.0))
